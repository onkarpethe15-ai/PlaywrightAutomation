...
const USER_EMAIL = "mailtonkar@gmail.com";
const USER_PASSWORD = "Panda@2025";
...
test("Banner IS visible when 6 events are returned", async ({ page }) => {
  await page.route("**/api/events**", async (route, request) => {
    let data = await JSON.stringify(SIX_EVENTS_RESPONSE);
    await route.fulfill({
      status: 200,
...
test("Banner is NOT visible when 4 events are returned", async ({ page }) => {
  await page.route("**/api/events**", async (route, request) => {
    const data_two = JSON.stringify(FOUR_EVENTS_RESPONSE);
