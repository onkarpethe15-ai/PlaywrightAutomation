...
async function booking(page, bookingcount) {
...
test("Single ticket booking eligible for refund", async ({ page }) => {
  ...
  const cancelbookingbutton = page.getByRole("button", {
  ...
  await expect(booking_id.charAt(0)).toMatch(show_title.charAt(0));
  await page
  ...
  const refund_txt = page.locator("//div[@id='refund-result']/span");
