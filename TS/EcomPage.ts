...
class EcomPage {
  ...
  async login(userNamne: string, passWord: string) {
    await this.emailinput.fill(userNamne);
    await this.passfiled.fill(passWord);
    await this.submitbtn.click();
  }

  async addToCart(itemtoadd: string) {
    const count = await this.elements.count();
    for (let i = 0; i < count; i++) {
      let texts: any;
      texts = await this.elements.nth(i).locator("h5").textContent();
      console.log(texts);
  ...
  async performCheckout() {
    ...
    let purchaseid: any;
    purchaseid = await this.purchaseidb.textContent();
